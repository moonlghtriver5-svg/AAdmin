...
function fillTemplate(template: string): string {
  const replacements: Record<string, string[]> = {
    ...
    '{threat}': ['competitive intelligence gathering', 'data exfiltration', 'unauthorized access'],
    '{round}': ['B', 'C', 'A'],
    ...
    '{assumptions}': ['40% automation rate', '60% ticket deflection', '30% efficiency gain'],
    '{company_type}': ['Fortune 500 enterprises', 'mid-market SaaS companies', 'healthcare providers'],
    '{providers}': ['OpenAI, Anthropic, Google', '3 cloud providers', 'multi-cloud setup'],
    '{regions}': ['North America, Europe, Asia', 'global markets', 'EMEA and APAC'],
    '{languages}': ['English, Spanish, Mandarin', '20+ languages', 'European languages'],
    '{num_models}': ['50+', '100+', '25+'],
    '{model}': ['GPT-4', 'Claude 3', 'custom fine-tuned model'],
    '{growth}': ['10k records/day', '100GB/month', '5k documents/week'],
    '{spend}': ['50k', '100k', '75k', '200k'],
  };
...
async function generateData(count: number = 500) {
  console.log(`🚀 Generating ${count} realistic AI interactions using OpenRouter...`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(`\n[${i + 1}/${count}] Generating response for ${user.name} using ${model}...`);
    console.log(`Prompt: ${prompt.substring(0, 100)}...`);

    const { response, tokens } = await callOpenRouter(prompt, model);

    // Determine if should be flagged
    const shouldFlag = Math.random() < 0.15; // 15% chance
    ...
    // Save incrementally every 10 logs
    if (logs.length % 10 === 0) {
      ...
      console.log(`💾 Saved ${logs.length} logs so far...`);
    }
...
generateData(count).catch(console.error);
